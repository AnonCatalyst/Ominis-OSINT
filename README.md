...
-- **BUGS:**
#1, `Also if Google search returns no results please retry the search by re-running the tool, because the tool tends to be blocked with some search attempts. Serpapi is planned to be introduced as an optional choice if you'd like to not use the built in search.`⏳️
#2,
